...
from src.utils.visual_odometer import VisualOdometer
...
class Tracker(object):
    def __init__(self, cfg, args, slam
        ...
        self.visual_odometry = cfg['tracking']['visual_odometer']
        ...
        if self.visual_odometry:
            ...
            manager = BaseManager()
            manager.start()
            self.visual_odometer = manager.VisualOdometer(
                [-1, -1, self.fx, self.fy, self.cx, self.cy], device=self.device)

    def set_pipe(self, pipe):
        self.pipe = pipe

    def optimize_cam_in_batch(self, camera_tensor, gt_color, gt_depth, batch_size, optimizer,
                              selected_index=None, K=None, kf_dict=None, gt_c2w=None):
        """
    ...
    def update_para_from_mapping(self):
        ...
        if self.mapping_idx[0] != self.prev_mapping_idx:
            ...
            self.cloud_pos = torch.tensor(
                self.npc.get_cloud_pos(), device=self.device).reshape(-1, 3)
            if self.verbose:
    ...
    def run(self, time_string):
        ...
        for idx, gt_color, gt_depth, gt_c2w in pbar:
            ...
            if (idx > 1 and idx % self.every_frame == 1):
                m_idx = self.pipe.recv()
                pre_c2w = self.estimate_c2w_list[idx-1].to(device)
                if self.visual_odometry:
                    self.visual_odometer.update_est_abs_pose(
                        pre_c2w.detach().clone().cpu().numpy())
            ...
            if idx <= 1 or self.gt_camera:
                ...
                if self.visual_odometry:
                    self.visual_odometer.set_init_state(
                        gt_color.detach().clone().cpu().numpy(),
                        gt_depth.detach().clone().cpu().numpy(),
                        gt_c2w.detach().clone().cpu().numpy())
            else:
                ...
                elif self.visual_odometry and idx-2 >= 0:
                    estimated_new_cam_c2w = self.visual_odometer.estimate_pose(
                        gt_color.clone().detach().cpu().numpy(),
                        gt_depth.clone().detach().cpu().numpy())
                    estimated_new_cam_c2w = torch.from_numpy(
                        estimated_new_cam_c2w).float().to(self.device)
                else:
                ...
                # optim_para_list.append({"params" : self.decoders.color_decoder.embedder.parameters(), "lr" : 0.001})
                # optim_para_list.append({"params" : self.decoders.color_decoder.embedder_rel_pos.parameters(), "lr": 0.001})

                # optim_para_list.append({"params" : self.decoders.geo_decoder.embedder.parameters(), "lr" : 0.001})
                # optim_para_list.append({"params" : self.decoders.geo_decoder.embedder_rel_pos.parameters(), "lr":0.001})
                ...
                for cam_iter in range(self.num_cam_iters):
                    ...
                    loss, color_loss_pixel, geo_loss_pixel = self.optimize_cam_in_batch(camera_tensor, gt_color, gt_depth, self.tracking_pixels,
                                                                                        ...
                                                                                        K=K, kf_dict=self.keyframe_dict, gt_c2w=gt_c2w)
