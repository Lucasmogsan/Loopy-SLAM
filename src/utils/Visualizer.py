    ...
    def vis(self, idx, iter, gt_depth, gt_color, c2w_or_camera_tensor, npc,
        ...
        with torch.no_grad():
            ...
            if conditions:
                ...
                cv2.imwrite(os.path.join(f'{self.vis_dir}',f'frame_{idx:05d}.png'), img)
                cv2.imwrite(os.path.join(f'{self.vis_dir}',f'frame_depth_{idx:05d}.png'), depth.detach().cpu().numpy())
